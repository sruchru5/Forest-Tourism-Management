...
def login():
        if e1.get()=="" or e2.get()=="":
                ...
                cur = con.cursor()
                cur.execute("select * from tbluser where uname=%s and upwd = %s",(e1.get(),e2.get()))
                row = cur.fetchone()
                if row==None:
...
labelb = Label(root,text ="USER LOGIN",foreground="red",background='white',font =('Verdana',20))
